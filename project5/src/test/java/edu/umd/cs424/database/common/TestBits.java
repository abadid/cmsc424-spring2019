...
public class TestBits {
    ...
    public void testSetBitOnBytes() {
        ...
        for (int i = 0; i < 16; ++i) {
            Bits.setBit(buf, i, Bits.Bit.ONE);
        ...
        for (int i = 0; i < 16; ++i) {
            Bits.setBit(buf, i, Bits.Bit.ZERO);
            assertArrayEquals(expectedsZero[i], bytes);
            assertEquals(0, buf.position());
        }

        // Write 1's with 1 byte offset.
        buf.position(1);
        byte[][] expectedsOffset = {
            ...
            {(byte) 0b00000000, (byte) 0b11111110},
            {(byte) 0b00000000, (byte) 0b11111111},
        };
        for (int i = 0; i < 8; ++i) {
            Bits.setBit(buf, i, Bits.Bit.ONE);
            assertArrayEquals(expectedsOffset[i], bytes);
